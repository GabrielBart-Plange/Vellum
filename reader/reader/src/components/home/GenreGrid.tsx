            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {GENRES.map((genre) => (
                    ...
                    >
                        ...
                        <div className="absolute inset-0 bg-[var(--reader-bg)]/40 z-10" />
                        ...
                        {/* Stacked Layers */}
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-36 bg-[var(--reader-surface)] rounded-lg shadow-2xl transition-all duration-700 group-hover:rotate-6 group-hover:translate-x-4 border border-white/5" />
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-36 bg-[var(--reader-surface)] rounded-lg shadow-2xl transition-all duration-700 group-hover:-rotate-6 group-hover:-translate-x-4 border border-white/5" />
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-36 bg-[var(--reader-surface-hover)] rounded-lg shadow-2xl z-20 border border-white/10 overflow-hidden">
                            <div className={`absolute inset-0 bg-gradient-to-t from-black/80 to-transparent`} />
                        </div>

                        <div className="absolute inset-0 z-30 p-6 flex flex-col justify-end">
                            <h3 className="text-lg font-bold text-[var(--reader-text)] tracking-widest uppercase">{genre.name}</h3>
                            <p className="text-[10px] text-[var(--reader-text-muted)] font-bold uppercase tracking-widest group-hover:text-[var(--reader-accent)] transition-colors">Discover</p>
                        </div>
                    </Link>
                ))}
            </div>
