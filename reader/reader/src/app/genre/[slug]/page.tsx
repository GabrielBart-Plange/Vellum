...
export default function GenrePage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const loadWorks = async (isMore = false) => {
        ...
        try {
            ...
            console.error("Error fetching works by genre:", err);
        } finally {
            setLoading(false);
            ...
            <div className="max-w-7xl mx-auto space-y-20">
                ...
                <header className="space-y-8 border-l-2 border-[var(--reader-accent)]/30 pl-10">
                    <div className="space-y-2">
                        ...
                        <h1 className="text-6xl md:text-9xl font-black tracking-tighter text-white italic uppercase leading-none">{genreName}</h1>
                    </div>
                    <p className="text-[var(--reader-text-muted)] max-w-2xl text-sm leading-relaxed italic">
                        All sagas and chronicles resonating within the {genreName} theme.
                    </p>
                    ...
                    <div className="flex flex-wrap gap-4 pt-4 items-center">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <select 
                                value={sortBy}
                                onChange={(e) => setSortBy(e.target.value as 'createdAt' | 'likes' | 'views')}
                        ...
                        <div className="col-span-full py-32 text-center glass-panel rounded-[2.5rem] border-dashed border-white/10 bg-white/[0.01]">
                            <div className="max-w-md mx-auto space-y-4">
                                ...
                                <p className="text-zinc-600 text-[9px] uppercase tracking-widest font-bold">
                                    No {activeTab === 'all' ? 'works' : activeTab + 's'} have been published in the {genreName} Library yet.
                                </p>
                            </div>
                        </div>
                    )}
